# OS_Millforest_Tickets — Executive Overview

A Power BI Project (PBIP) containing a **semantic model** and a single-page **Executive Overview** report for **Millforest Dental Group** service-desk ticket health, built on the Syncro RMM PostgreSQL export.
...
2. Open **`OS_Millforest_Tickets.pbip`**.
...
`definition/expressions.tmdl` holds one parameter, **`CustomerId` = 12315250** (`Millforest Dental Group`). The `Tickets` and `Customers` Power Query steps both filter to it, so the model only ever contains this client's data — the filter folds to Postgres.
