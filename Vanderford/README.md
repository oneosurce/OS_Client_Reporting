# OS_Vanderford_Tickets — Executive Overview

A Power BI Project (PBIP) containing a **semantic model** and a single-page **Executive Overview** report for **Vanderford Air and Plumbing** service-desk ticket health, built on the Syncro RMM PostgreSQL export.
...
2. Open **`OS_Vanderford_Tickets.pbip`**.
...
`definition/expressions.tmdl` holds one parameter, **`CustomerId` = 34611186** (`Vanderford Air and Plumbing`). The `Tickets` and `Customers` Power Query steps both filter to it, so the model only ever contains this client's data — the filter folds to Postgres.
