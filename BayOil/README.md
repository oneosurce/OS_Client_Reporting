# OS_BayOil_Tickets — Executive Overview

A Power BI Project (PBIP) containing a **semantic model** and a single-page **Executive Overview** report for **Bay Oil Co.** service-desk ticket health, built on the Syncro RMM PostgreSQL export.
...
`definition/expressions.tmdl` holds one parameter, **`CustomerId` = 12432163** (`Bay Oil Co.`). The `Tickets` and `Customers` Power Query steps both filter to it, so the model only ever contains this client's data — the filter folds to Postgres.
