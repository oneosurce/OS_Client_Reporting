# OS_MartinWalton_Tickets — Executive Overview

A Power BI Project (PBIP) containing a **semantic model** and a single-page **Executive Overview** report for **Martin Walton Attorneys at Law** service-desk ticket health, built on the Syncro RMM PostgreSQL export.

> One client folder in the [`OS_Client_Reporting`](../README.md) monorepo (siblings: [`FWW`](../FWW/), [`Sophia`](../Sophia/)). All client dashboards are identical apart from the `CustomerId` parameter and the client name in the header/footer.
...
2. Open **`OS_MartinWalton_Tickets.pbip`**.
...
`definition/expressions.tmdl` holds one parameter, **`CustomerId` = 12309358** (`Martin Walton Attorneys at Law`). The `Tickets` and `Customers` Power Query steps both filter to it, so the model only ever contains this client's data — the filter folds to Postgres.
